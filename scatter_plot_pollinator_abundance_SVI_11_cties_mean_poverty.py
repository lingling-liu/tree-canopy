...
for file_path in glob.glob(os.path.join(input_folder, "*.csv")):
    ...
    # Create a scatter plot for mean pollinator abundance vs EP_POV150
    fig, ax = plt.subplots(figsize=(6, 6))
    ax.scatter(filtered_data['EP_POV150'], filtered_data['mean'], alpha=0.5, label='Cities')
    ax.set_xlabel('EP_POV150', fontsize=14)
    ax.set_ylabel('Mean Pollinator Abundance', fontsize=12)
    ax.set_title(f'{city_name}', fontsize=14)
    ax.tick_params(axis='x', labelsize=14)
    ax.tick_params(axis='y', labelsize=14)
    r_squared, p_value = add_trendline(ax, filtered_data['EP_POV150'], filtered_data['mean'])
...
combined_file_name = os.path.join(output_folder, "combined_mean_pollinator_EP_POV150.png")
