...
# Input files
diff_file = r"G:\Shared drives\Urban Workflow\Data\Urban_LTER\Pollination\06-26-2024\pollinator_abundance_difference.tif"
...
with rasterio.open(aligned_canopy_file) as src1, rasterio.open(aligned_diff_file) as src2:
    canopy = src1.read(1)
    poll_diff = src2.read(1)

    if canopy.shape != poll_diff.shape:
...
mask = (~np.isnan(canopy)) & (~np.isnan(poll_diff)) & (canopy > 0)

# Flatten and apply mask
canopy_vals = canopy[mask].flatten()
poll_diff_vals = poll_diff[mask].flatten()

# Sample 20,000 points
print("Sampling 20,000 points...")
sample_size = min(20000, len(canopy_vals))
canopy_sample, poll_diff_sample = resample(
    canopy_vals, poll_diff_vals, n_samples=sample_size, random_state=42
)
...
plt.scatter(canopy_sample, poll_diff_sample, alpha=0.2, s=10, label='Sample Points')
plt.plot(bins[:-1] + 2.5, grouped['PollDiff'], color='red', lw=2, marker='o', label='Average per 5% Bin')
plt.xlabel("Tree Canopy % (within 500m)")
plt.ylabel("Pollinator Abundance Difference (NLCD+trees − NLCD only)")
plt.title("Pollination Difference vs. Tree Canopy %")
plt.grid(True)
plt.legend()
plt.tight_layout()

# Save the figure as a 300 DPI PNG image
output_path = r"G:\Shared drives\Urban Workflow\Data\Urban_LTER\Figure\pollination_diff_vs_canopy_052025.png"
