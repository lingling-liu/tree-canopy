...
# OPTIONAL: write row-level diffs / dropped rows
...
OUT_DROPPED_A    = r"E:\FInal\N_species\polygons\species_by_country\dropped_rows_A.csv"
OUT_DROPPED_B    = r"E:\FInal\N_species\polygons\species_by_country\dropped_rows_B.csv"
...
# ============================================================
# REPORT: WHY ROW COUNT DROPPED
# ============================================================
def get_dropped_rows(df_raw: pd.DataFrame, name: str) -> pd.DataFrame:
    """
    Re-normalize raw dataframe (without dropping) and return rows that would be dropped
    because WB_NAME or species is missing after normalization.
    """
    raw = df_raw.copy()
    ...
    if missing:
        raise ValueError(
            f"[{name}] Missing required columns: {missing}\n"
            f"[{name}] Found columns: {list(raw.columns)}"
        )

    for c in ["WB_NAME", "Group", "species"]:
        raw[c] = raw[c].map(normalize_text)

    raw["Group"] = raw["Group"].fillna("Unknown")

    dropped = raw[raw["WB_NAME"].isna() | raw["species"].isna()].copy()
    return dropped[["WB_NAME", "Group", "species"]]

def print_and_save_dropped(df_raw: pd.DataFrame, name: str, out_path: str, max_print: int = 30):
    dropped = get_dropped_rows(df_raw, name)

    print(f"\n================ {name}: DROPPED ROWS ================")
    print(f"Dropped rows count: {len(dropped)}")

    if dropped.empty:
        print("✅ No rows dropped.")
        return dropped

    print("\nDropped rows (first rows):")
    print(dropped.head(max_print).to_string(index=False))

    if out_path:
        os.makedirs(os.path.dirname(out_path), exist_ok=True)
        dropped.to_csv(out_path, index=False, encoding="utf-8")
        print(f"\nWrote dropped rows to:\n{out_path}")
...
# ============================================================
# DUPLICATE + ROW COUNT REPORTING
# ============================================================
def report_rows_and_duplicates(df_raw: pd.DataFrame, df_std: pd.DataFrame, name: str):
    raw_rows = len(df_raw)
    std_rows = len(df_std)

    key_cols = ["WB_NAME", "Group", "species"]
    unique_keys = df_std.drop_duplicates(key_cols)
    n_unique_keys = len(unique_keys)

    dup_mask = df_std.duplicated(key_cols, keep=False)
    dup_rows = df_std.loc[dup_mask].copy()
    n_dup_rows = len(dup_rows)
    dup_excess = std_rows - n_unique_keys

    print(f"\n================ {name}: ROWS & DUPLICATES ================")
    print(f"Raw rows in file: {raw_rows}")
    print(f"Rows after standardize(): {std_rows}")
    print(f"Unique (WB_NAME, Group, species) keys: {n_unique_keys}")
    print(f"Duplicate rows (same key repeated): {n_dup_rows}")
    print(f"Duplicate EXCESS rows (std_rows - unique_keys): {dup_excess}")

    if n_dup_rows > 0:
        top_dups = (
            dup_rows.groupby(key_cols)
            .size()
            .reset_index(name="times_seen")
            .sort_values("times_seen", ascending=False)
            .head(20)
        )
        print("\nTop duplicate keys (up to 20):")
        print(top_dups.to_string(index=False))
    else:
        print("✅ No duplicate keys found in standardized data.")
...
# ---- NEW: show exactly which rows will be dropped (before standardize drops them) ----
dropped_a = print_and_save_dropped(df_a_raw, "Table A (intersection)", OUT_DROPPED_A, max_print=30)
dropped_b = print_and_save_dropped(df_b_raw, "Table B (ArcGIS)", OUT_DROPPED_B, max_print=30)
...
# Report rows and duplicates
report_rows_and_duplicates(df_a_raw, df_a, "Table A (intersection)")
report_rows_and_duplicates(df_b_raw, df_b, "Table B (ArcGIS)")
...
a_only_rows, b_only_rows = print_row_diffs(df_a=df_a, df_b=df_b, max_pairs=20, max_rows_each=30)
