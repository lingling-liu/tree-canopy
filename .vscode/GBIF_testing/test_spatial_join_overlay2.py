...
def report_rows_and_duplicates(df_raw: pd.DataFrame, df_std: pd.DataFrame, name: str):
    ...
    Prints:
      - raw row count
      - standardized row count (after cleaning)
      - unique key count (WB_NAME, Group, species)
      - duplicate rows count (same key appears multiple times)
      - top duplicate keys (if any)
    """
    raw_rows = len(df_raw)
    std_rows = len(df_std)

    # key duplicates in standardized
    ...
    # "extra" beyond unique: how many rows are repeated
...
# ---- NEW: report rows and duplicates for each original table ----
