pro generate_table_tree_canopy_nasa_joe
  ...
  index_roi = where(data_mask eq 1 and data_Joe ne -9999, count)
  X = data_tree[index_roi]
  Y = fix(data_Joe[index_roi]*100)
  print, min(X), max(X)
  print,min(Y),max(Y)
  
  result = intarr(2, n_elements(X))-9999
  result[0,*] = X
  result[1,*] = Y
  ...
  file_name = "Y:\people\liu02034\Tree_canopy\table_NASA_Joe_tree_canopy.csv"
