...
ground_temp = "D:/Shared drives/Urban Workflow/UCM TCMA/Existing_Data/resize/reproject/JJA_Night_Temp1_resize_reproject.tif"
...
directory =  "D:/Shared drives/Urban Workflow/Minneapolis/New_NLCD/New_air_temperature/reproject/tif/"
...
for filename in os.listdir(directory):
    ...
    # Load the two rasters
        ...
        textstr = '\n'.join((
            r'$R^2=%.2f$' % (r_value**2,),
            r'$P=%.2f$' % (p_value,)))
        ax[i].text(19.5, 23.5, textstr, fontsize=14, verticalalignment='top')

        ax[i].set_xlabel('Ground Night temp')
        ax[i].set_ylabel(title_list[i])

        # set the x and y axis ranges
        # ax[i].set_xlim(24, 25.6)
        ax[i].set_ylim(23.25, 25)
...
#plt.savefig("scatter_plot_ground_night_modelled.png",dpi =300)
plt.savefig("heatmap_ground_night_modelled.png",dpi =300)